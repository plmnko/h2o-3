  ...
  def _model_build(self, x, y, tframe, vframe, kwargs):
    ...
    if '_rest_version' in list(kwargs.keys()):
      ...
      for error_message in grid_json["failure_details"]:
        print("Errors/Warnings building gridsearch model!\nHyper-parameter settings: names {0}\nvalues: {1}\n"
              "failure_details: {2}\n"
              "failure_stack_traces: {3}\n".format(grid_json['hyper_names'],
                                                   grid_json['failed_raw_params'][error_index],
                                                   error_message, grid_json['failure_stack_traces'][error_index]))
        error_index += 1
